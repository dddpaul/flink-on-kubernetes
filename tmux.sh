...
tmux split-window -h
tmux send-keys -t $session 'watch -n1 "kubectl top pods --sum=true -l \"type in (flink-native-kubernetes)\""' Enter
tmux select-pane -L
tmux send-keys -t $session 'watch -n1 "kubectl get pods -l \"type in (flink-native-kubernetes)\" -o custom-columns=\"NAME:.metadata.name,STATUS:.status.phase,RESTARTS:.status.containerStatuses[0].restartCount,NODE:.spec.nodeName\""' Enter
tmux select-pane -L
